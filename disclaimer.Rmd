...
company %<>% 
  ...
  mutate(PSR = Price / Revenue_per_share,
         ...
         Decision = case_when(BVPS < 0 ~ "Negative equity",
                              ...
                              sum(c_across(c(PEG, `Price/Graham`, `Price/DCF10`, `Marketcap/Payback8`)) < 1, `Price/Target` < 0.8, na.rm = T) >= 3 ~ "Buy",
                              sum(c_across(c(PEG, `Price/Graham`, `Price/DCF10`, `Marketcap/Payback8`)) < 1, `Price/Target` < 0.8, na.rm = T) >= 2 ~ "Maybe buy",
...
**Disclosure**: I and my wife own shares of `r str_c(str_c(my_stock[-length(my_stock)], collapse = ", "), my_stock[length(my_stock)], sep = ", and ")`.
